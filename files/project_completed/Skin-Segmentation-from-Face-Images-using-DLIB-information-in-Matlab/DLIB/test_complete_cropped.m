...
fail_count=fail_count+1;
  ...
  rgb_cropped_gray=rgb2gray(rgb_cropped);
    ...
    f_new = detector('detect', h, rgb_cropped);
    r_new = zeros(size(f_new, 1), 64, 64, 3, 'uint8');
    s_new = zeros(size(f_new, 1), 68, 2);
  for i = 1:size(f_new, 1)
    s_new(i, :, :) = detector('fit', h, rgb_cropped, f_new(i, :));
    r_new(i, :, :, :) = extractroi(rgb_cropped, s(i, :, :), params);
  end

  
%% ITA Equation
ITA=fun_get_ITA(rgb_cropped,0,'f');

lab=rgb2lab(rgb_cropped);
Lab_L = lab(:,:,1);
Lab_a = lab(:,:,2);
Lab_b = lab(:,:,3);
%% Averaging Filter
...
% % % % % % % % % % % % % % % % % % % % % % % % 
figure(1),imshow(rgb_cropped_gray),title('Gray Image with Parts Plotted');
hold on; axis on;
for i = 1:size(f, 1)
    x = [f_new(i, 1), f_new(i, 3), f_new(i, 3), f_new(i, 1), f_new(i, 1)];
    y = [f_new(i, 2), f_new(i, 2), f_new(i, 4), f_new(i, 4), f_new(i, 2)];
    plot(x, y, '-r');
    for j=1:68
        box_x(j,1)=s_new(i, j, 1);
        box_y(j,1)=s_new(i, j, 2);
    end
...
% saveas(gcf,['1_',in_file,'.jpg']);
%% Cropping Image
BW1=roipoly(rgb_cropped,box_cheek_right(1,:),box_cheek_right(2,:));
BW2=roipoly(rgb_cropped,box_cheek_left(1,:),box_cheek_left(2,:));
BW3=roipoly(rgb_cropped,box_chin(1,:),box_chin(2,:));
BW4=roipoly(rgb_cropped,box_forehead(1,:),box_forehead(2,:));
...
out1 = rgb_cropped .* uint8(BW1);
out2 = rgb_cropped .* uint8(BW2);
out3 = rgb_cropped .* uint8(BW3);
out4 = rgb_cropped .* uint8(BW4);
...
figure(3),
subplot(121),hist(ITA_Sum)
subplot(122),hist(ITA_Sum_Smooth)

%% Plotting Full ITA
alpha = (~isnan(ITA))*0.6;
figure(4)
imshow(rgb_cropped),title('ITA Plotted on Origal RGB');
hold on
