...
def ensemble_models(i,x_train, y_train,x_test,y_test,classes):
    model=[0]*(i+3)
    model[i]=cf0(x_train, y_train, x_test, y_test,classes)
    model[i+1]=cf1(x_train, y_train, x_test, y_test,classes)
    model[i+2]=cf2(x_train, x_test, y_train,y_test,classes)
    return model
