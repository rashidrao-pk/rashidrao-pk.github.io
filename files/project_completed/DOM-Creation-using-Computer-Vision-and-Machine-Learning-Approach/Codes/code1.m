...
% values into 'GroundTruth_Desc'
GroundTruth_Desc = readtable('Coordinators of Image 01.xlsx', 'ReadVariableNames', true);

% GroundTruth_Desc_Sorted = sortrows(GroundTruth_Desc,{'Height','Weight'},{'ascend','descend'});
% GroundTruth_Desc_Sorted_1 = sortrows(GroundTruth_Desc,{'yAxis','xAxis'},{'ascend','ascend'});
% GroundTruth_Desc_Sorted = sortrows(GroundTruth_Desc,["yAxis","xAxis"]);
GroundTruth_Desc_Sorted=GroundTruth_Desc;
% GroundTruth_Desc_Sorted = sortrows(GroundTruth_Desc,["yAxis","xAxis"]);
...
%   all the components.
[max_x,index_x]=max(GroundTruth_Desc.xAxis);
[max_y,index_y]=max(GroundTruth_Desc.yAxis);
[max_w,index_w]=max(GroundTruth_Desc.width);
[max_h,index_h]=max(GroundTruth_Desc.height);
...
% not ??
    if (GroundTruth_Desc.Component(i,1) == finalName{i,1})
%   Displaying the Image name and the Coordinates file have the same name or not
%         disp([GroundTruth_Desc.Component(i,1) ' == ' finalName{i,1}]);
%       Saving Coordinates values into varibale
        pos_x=GroundTruth_Desc.xAxis(i,1);
        pos_y=GroundTruth_Desc.yAxis(i,1);
        pos_w=GroundTruth_Desc.width(i,1);
        pos_h=GroundTruth_Desc.height(i,1);
...
% GroundTruth_Desc_Sorted

for i=1:total_inner_Components
%     Getting Image Location for image at i index
    clear component_name
    component_name=GroundTruth_Desc_Sorted.Component(i,1);
    x_Axis=GroundTruth_Desc_Sorted.xAxis(i,1);    
    y_Axis=GroundTruth_Desc_Sorted.yAxis(i,1);
    height=GroundTruth_Desc_Sorted.height(i,1);
    width=GroundTruth_Desc_Sorted.width(i,1);
    
    if(i==total_inner_Components)
    x1=GroundTruth_Desc_Sorted.xAxis(i,1);
    y1=GroundTruth_Desc_Sorted.yAxis(i,1);
    w1=GroundTruth_Desc_Sorted.width(i,1);
    h1=GroundTruth_Desc_Sorted.height(i,1);
    else
    x1=GroundTruth_Desc_Sorted.xAxis(i+1,1);
    y1=GroundTruth_Desc_Sorted.yAxis(i+1,1);
    w1=GroundTruth_Desc_Sorted.width(i+1,1);
    h1=GroundTruth_Desc_Sorted.height(i+1,1);
    end

%     disp([ num2str(x_Axis + width) ' > ' num2str(x1 + w1)]);
%     disp([ num2str(y_Axis + height+ x_Axis) ' > ' num2str(y1+ h1)]);
%     
%     if  y_Axis == GroundTruth_Desc_Sorted.yAxis(i+1,1)
...
%     end
    ...
    GroundTruth_Desc_Sorted_New{i,1}=component_name;
%     GroundTruth_Desc_Sorted_New{i,2}= GroundTruth_Desc_Sorted.xAxis(i,1)+ GroundTruth_Desc_Sorted.width(i,1);    
%     GroundTruth_Desc_Sorted_New{i,3}= GroundTruth_Desc_Sorted.yAxis(i,1)+ GroundTruth_Desc_Sorted.height(i,1);
        GroundTruth_Desc_Sorted_New{i,2}= x_Axis + y_Axis+ height;  
        GroundTruth_Desc_Sorted_New{i,3}= (x_Axis + x1) - (y_Axis + y1) ;
%         GroundTruth_Desc_Sorted_New{i,4}= y_Axis -y1 ;
...
GroundTruth_Desc_Sorted_New = sortrows(GroundTruth_Desc_Sorted_New,3,'ascend');